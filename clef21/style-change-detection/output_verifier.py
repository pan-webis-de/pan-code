...
class ParseError(Enum):
    ...
    TASK3_INVALID_FORMAT = 'Task 3 solution has to be numeric array with author numbers 1-5.'
    TASK3_INVALID_LENGTH = 'Task 3 solution array length does not match number of paragraphs.'
...
def get_solution_file_check_result(solution_file, problem_id, input_folder):
    ...
    if 'paragraph-authors' not in solution:
        warnings.warn(f"[problem {problem_id}]: Task 3 solution missing (WARNING).", SyntaxWarning)
    # solution for task 3 has to be list
    elif (not isinstance(solution['paragraph-authors'], list)):
            occurred_errors.append(ParseError.TASK3_INVALID_FORMAT)
    else:
        # solution for task 3 list may only contain integers <= 5 (5 different authors).
        if not all(i <= 5 and i > 0 for i in solution['paragraph-authors']):
           ...
           if len(solution['paragraph-authors']) != (get_paragraph_count(problem_id, input_folder)):
