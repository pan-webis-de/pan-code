    ...
    for problem_id in problem_ids:
        ...
        if os.path.exists(file_path):
            ...
            if not errors:
                ...
                [print(f"[problem {problem_id}: {task.value}]: ERROR {x.value}")
                 for x in errors]
        else:
            print(
...
def main():
    parser = argparse.ArgumentParser(
        description="PAN25 Style Change Detection Task: Output Verifier"
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    # task 1
    problem_ids = get_problem_ids(os.path.join(args.input, "easy"))
    check_output_files(problem_ids, os.path.join(
        args.output, "easy"), os.path.join(args.input, "easy"), Task.TASK1)

    # task 2
    problem_ids = get_problem_ids(os.path.join(args.input, "medium"))
    check_output_files(problem_ids, os.path.join(
        args.output, "medium"), os.path.join(args.input, "medium"), Task.TASK2)

    # task 3
    problem_ids = get_problem_ids(os.path.join(args.input, "hard"))
    check_output_files(problem_ids, os.path.join(
        args.output, "hard"), os.path.join(args.input, "hard"), Task.TASK3)
