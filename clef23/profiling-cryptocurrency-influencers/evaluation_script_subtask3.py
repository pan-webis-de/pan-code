...
"""
# Evaluation script for the Profiling Cryptocurrency Influencers with Few-shot Learning task @PAN2023 - Low-resource influencer intent identification - subtask3.
## Measures
...
``` json
    {"twitter user id":"2f5a43bbfb7fcb3623222de6305b45b9","class":"subjective opinion"}
    {"twitter user id":"5509d8b2ba81eb6bc580a8aaa89b335a","class":"financial information"}
    
```
path-to-model-output - subtask3.json: e.g.
``` json
    {"twitter user id":"2f5a43bbfb7fcb3623222de6305b45b9","class":"subjective opinion", "probability": 0.99 }
    {"twitter user id":"5509d8b2ba81eb6bc580a8aaa89b335a","class":"financial information", "probability": 0.65}
...
where
    ...
    path-to-model-output is the path to the file of the 'subtask3.json' file for a submitted method
    OUTPUT is the path to the folder where the results of the evaluation will be saved
Example: 
>>> python pan23-author-profiling-evaluator-subtask3.py -g "subtask3/test_truth.json" \
        -s "submitted_models/subtask3.json" \
        -o "output_evaluation_subtask3"
...
TASKS_LABELS = {"subtask3": ["announcement", "subjective opinion", "financial information", "advertising"]}
...
def write_evaluations(results: dict, output_directory: Path):
    ...
    try:
        ...
        with open(output_directory.joinpath("evaluation.prototext"), 'w+') as of:
            of.write('measure{{\n  key: "{}"\n  value: "{}"\n}}\n'.format('subtask3', str(results['umF1'])))
        ##Write json
...
def main():
    parser = argparse.ArgumentParser(description='Evaluation script Author Profiling@PAN2023 - subtask3')
    parser.add_argument('-g', type=Path,
                        help='Path to test_truth.json that contains truth')
    parser.add_argument('-s', type=Path,
                        help='Path to subtask3.json that contains system predictions')
    parser.add_argument('-o', type=Path, help='Path to output files', default="./output_evaluation_subtask3")
    ...
    df_prediction = load_file(args.s.joinpath("subtask3.json"))
    # evaluation
