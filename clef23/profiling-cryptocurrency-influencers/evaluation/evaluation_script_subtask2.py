...
"""
# Evaluation script for the Profiling Cryptocurrency Influencers with Few-shot Learning task @PAN2023 - Low-resource influencer interest identification - subtask2.
## Measures
...
``` json
    {"twitter user id":"2f5a43bbfb7fcb3623222de6305b45b9","class":"technical information"}
    {"twitter user id":"5509d8b2ba81eb6bc580a8aaa89b335a","class":"macro"}
    
```
path-to-model-output - subtask2.json: e.g.
``` json
    {"twitter user id":"2f5a43bbfb7fcb3623222de6305b45b9","class":"technical information", "probability": 0.99 }
    {"twitter user id":"5509d8b2ba81eb6bc580a8aaa89b335a","class":"price update", "probability": 0.65}
...
where
    ...
    path-to-model-output is the path to the file of the 'subtask2.json' file for a submitted method
    OUTPUT is the path to the folder where the results of the evaluation will be saved
Example: 
>>> python pan23-author-profiling-evaluator-subtask2.py -g "subtask2/test_truth.json" \
        -s "submitted_models/subtask2.json" \
        -o "output_evaluation_subtask2"
...
TASKS_LABELS = {"subtask2": ["other", "technical information", "trading matters", "price update", "gaming"]}
...
def write_evaluations(results: dict, output_directory: Path):
    ...
    try:
        ...
        with open(output_directory.joinpath("evaluation.prototext"), 'w+') as of:
            of.write('measure{{\n  key: "{}"\n  value: "{}"\n}}\n'.format('subtask2', str(results['umF1'])))
        ##Write json
...
def main():
    parser = argparse.ArgumentParser(description='Evaluation script Author Profiling@PAN2023 - subtask2')
    parser.add_argument('-g', type=Path,
                        help='Path to test_truth.json that contains truth')
    parser.add_argument('-s', type=Path,
                        help='Path to subtask2.json that contains system predictions')
    parser.add_argument('-o', type=Path, help='Path to output files', default="./output_evaluation_subtask2")
